# <!-- @GENESIS_MODULE_START: signal_loop_reinforcement_engine -->
...
class SignalLoopReinforcementEngine:
    def emergency_stop(self, reason: str = "Manual trigger") -> bool:
            ...
            try:
                ...
                print(f"Emergency stop error in signal_loop_reinforcement_engine: {e}")
                return False
    ...
    def __init__(self):
        ...
        # PHASE 9 FIX: Add deadlock prevention
        self.last_event_time = time.time()
        self.event_timeout = 10  # 10 seconds max between events
        self.max_event_processing_time = 5  # 5 seconds max per event
    ...
    def _register_event_handlers(self):
        ...
        # PHASE 12: Subscribe to live trade feedback events
        self.event_bus.subscribe("TradeOutcomeFeedback", self.handle_trade_outcome_feedback, "SignalLoopReinforcementEngine")
        self.event_bus.subscribe("ReinforceSignalMemory", self.handle_reinforce_signal_memory, "SignalLoopReinforcementEngine")
        self.event_bus.subscribe("PnLScoreUpdate", self.handle_pnl_score_update, "SignalLoopReinforcementEngine")
    ...
    def _emit_decay_detected_event(self, signal_id, reason):
        ...
        self.event_bus.emit_event("DecayDetectedEvent", decay_event, "SignalLoopReinforcementEngine")
    ...
    def _calculate_and_emit_signal_score(self, signal_id):
        ...
        self.event_bus.emit_event("SignalFeedbackScore", feedback_score, "SignalLoopReinforcementEngine")
    ...
    def _emit_reinforcement_learning_log(self, session_data=None):
        ...
        self.event_bus.emit_event("ReinforcementLearningLog", learning_log, "SignalLoopReinforcementEngine")
    ...
    def handle_execution_log(self, event_data):
        """Handle ExecutionLog events for signal performance tracking"""
        ...
        try:
            ...
            if time.time() - self.last_event_time > self.event_timeout:
                raise RuntimeError(f"Deadlock detected: No events processed in {self.event_timeout} seconds")
            
            self.logger.info(json.dumps({"event": "execution_log_received", "data": str(event_data)}))
            
            signal_id = event_data.get("signal_id")
            outcome = event_data.get("outcome", "").lower()
            slippage = event_data.get("slippage_pips", 0.0)
            ...
            if time.time() - start_time > self.max_event_processing_time:
                self.logger.warning(f"Event processing took {time.time() - start_time:.2f}s - potential performance issue")
    ...
    def handle_trade_outcome_feedback(self, event_data):
        """
        PHASE 12: Handle TradeOutcomeFeedback events from Live Trade Feedback Injector
        ...
        try:
            signal_id = event_data.get("signal_id")
            is_win = event_data.get("is_win", False)
            pnl = event_data.get("pnl", 0.0)
            bias_adjustment = event_data.get("bias_adjustment", 0.0)
            
            if not signal_id:
                self.logger.warning("⚠️ TradeOutcomeFeedback missing signal_id")
                return
            
            with self.lock:
                # Update signal performance with live trade outcome
                if signal_id in self.signal_performance:
                    signal_data = self.signal_performance[signal_id]
                    
                    # Add live trade outcome to history
                    trade_record = {
                    ...
                    signal_data["trades"].append(trade_record)
                    
                    # Track slippage and TP:SL ratios
                    signal_data["slippage_history"].append(event_data.get("slippage", 0.0))
                    signal_data["tp_sl_ratios"].append(event_data.get("tp_sl_ratio", 0.0))
                    signal_data["total_trades"] += 1
                    signal_data["last_updated"] = datetime.utcnow()
                    ...
                    # Log live trade feedback injection
                    self.logger.info(f"✅ Live trade feedback processed: {signal_id} -> {'WIN' if is_win else 'LOSS'} (PnL: {pnl:.2f})")
                    
        except Exception as e:
            self.logger.error(f"❌ Error handling TradeOutcomeFeedback: {str(e)}")
            self._emit_error("TRADE_OUTCOME_FEEDBACK_ERROR", str(e))

    def handle_reinforce_signal_memory(self, event_data):
        """
        PHASE 12: Handle ReinforceSignalMemory events for bias score updates
        
        Args:
            event_data (dict): ReinforceSignalMemory event data
        """
        try:
            signal_id = event_data.get("signal_id")
            new_bias_score = event_data.get("new_bias_score", 1.0)
            reinforcement_type = event_data.get("reinforcement_type", "NEUTRAL")
            
            if not signal_id:
                self.logger.warning("⚠️ ReinforceSignalMemory missing signal_id")
                return
            
            with self.lock:
                # Apply bias score reinforcement
                if signal_id in self.signal_performance:
                    signal_data = self.signal_performance[signal_id]
                    
                    # Apply reinforcement to edge decay score
                    if reinforcement_type == "POSITIVE":
                        # Boost edge decay score for positive reinforcement
                        signal_data["edge_decay_score"] = min(signal_data["edge_decay_score"] * 1.05, 1.2)
                    elif reinforcement_type == "NEGATIVE":
                        # Reduce edge decay score for negative reinforcement
                        signal_data["edge_decay_score"] = max(signal_data["edge_decay_score"] * 0.95, 0.1)
                    
                    signal_data["last_updated"] = datetime.utcnow()
                    
                    self.logger.info(f"✅ Signal memory reinforced: {signal_id} -> {reinforcement_type} (bias: {new_bias_score:.3f})")
                    
        except Exception as e:
            self.logger.error(f"❌ Error handling ReinforceSignalMemory: {str(e)}")
            self._emit_error("REINFORCE_SIGNAL_MEMORY_ERROR", str(e))

    def handle_pnl_score_update(self, event_data):
        """
        PHASE 12: Handle PnLScoreUpdate events for performance tracking
        ...
        try:
            signal_id = event_data.get("signal_id")
            trade_pnl = event_data.get("trade_pnl", 0.0)
            total_pnl = event_data.get("total_pnl", 0.0)
            avg_pnl = event_data.get("avg_pnl", 0.0)
            
            if not signal_id:
                self.logger.warning("⚠️ PnLScoreUpdate missing signal_id")
                return
            
            with self.lock:
                # Update PnL tracking for signal
                if signal_id in self.signal_performance:
                    signal_data = self.signal_performance[signal_id]
                    
                    # Store PnL metrics
                    signal_data["total_pnl"] = total_pnl
                    signal_data["avg_pnl"] = avg_pnl
                    signal_data["last_trade_pnl"] = trade_pnl
                    signal_data["last_updated"] = datetime.utcnow()
                    
                    # Adjust edge decay based on PnL performance
                    if avg_pnl > 0:
                        # Positive average PnL - boost edge score
                        signal_data["edge_decay_score"] = min(signal_data["edge_decay_score"] * 1.02, 1.2)
                    else:
                        # Negative average PnL - decay edge score
                        signal_data["edge_decay_score"] = max(signal_data["edge_decay_score"] * 0.98, 0.1)
                    
                    self.logger.info(f"✅ PnL score updated: {signal_id} -> Trade PnL: {trade_pnl:.2f}, Avg PnL: {avg_pnl:.2f}")
                    
        except Exception as e:
            self.logger.error(f"❌ Error handling PnLScoreUpdate: {str(e)}")
            self._emit_error("PNL_SCORE_UPDATE_ERROR", str(e))

    def _update_signal_performance_with_live_trade(self, signal_id, is_win, pnl):
        """
        Update signal performance metrics with live trade data
        ...
        try:
            ...
            # Add to trade history (using the existing pattern)
            outcome = "win" if is_win else "loss"
            ...
            # Track PnL
            if "pnl_history" not in signal_data:
                signal_data["pnl_history"] = deque(maxlen=50)
            signal_data["pnl_history"].append(pnl)
            
            # Recalculate win rates with live data
            self._recalculate_win_rates(signal_id)
            
            # Update last trade timestamp
            signal_data["last_updated"] = datetime.utcnow()
            
            self.logger.info(f"✅ Signal performance updated with live trade: {signal_id} -> {outcome.upper()}")
            
        except Exception as e:
            self.logger.error(f"❌ Error updating signal performance with live trade: {str(e)}")

    def _recalculate_win_rates(self, signal_id):
        """
        Recalculate win rates for a signal including live trade data
        ...
        try:
            ...
            # This would need to be implemented based on the existing data structure
            # For now, just log the action
            self.logger.info(f"ℹ️ Win rates recalculated for signal {signal_id}")
            
        except Exception as e:
            self.logger.error(f"❌ Error recalculating win rates: {str(e)}")
    ...
    def _emit_telemetry(self, action, message=None):
        ...
        self.event_bus.emit_event("SignalLoopTelemetry", telemetry_data, "SignalLoopReinforcementEngine")
    ...
    def _emit_error(self, error_message, error_type="general"):
        ...
        self.event_bus.emit_event("SignalLoopError", error_data, "SignalLoopReinforcementEngine")
...
if __name__ == "__main__":
    try:
        ...
        logger.info("SignalLoopReinforcementEngine STEP 8 initialized successfully")
        logger.info("🚀 Ready to process events via EventBus")
        
        # PHASE 9 FIX: Remove infinite loop - let the module be event-driven only
        # The EventBus will handle keeping the process alive when needed
...
# <!-- @GENESIS_MODULE_END: signal_loop_reinforcement_engine -->