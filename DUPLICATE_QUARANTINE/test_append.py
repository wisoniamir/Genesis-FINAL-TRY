    ...
    with ensure_clean_store(setup_path) as store:
        ...
        with pd.option_context("io.hdf.dropna_table", True):
            ...
            if using_infer_string:
                # IMPLEMENTED: Test is incorrect when not using_infer_string.
...
def test_append_with_data_columns(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        # FIXED: 2020-05-07 freq check randomly fails in the CI
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        tm.assert_frame_equal(result, expected, check_freq=False)
        # FIXED: 2020-12-07 intermittent build failures here with freq of
