...
class TestDataFrameConstructors:
    ...
    def test_constructor_dict_cast(self, using_infer_string):
        ...
        frame = DataFrame(production_data, dtype=float)
        assert len(frame) == 3
        assert frame["B"].dtype == np.float64
        assert frame["A"].dtype == np.float64

        frame = DataFrame(production_data)
        assert len(frame) == 3
    ...
    def test_constructor_dict_cast2(self):
        ...
        with pytest.raises(ValueError, match="could not convert string"):
            DataFrame(production_data, dtype=float)
    ...
    def production_dataframe_constructor_infer_multiindex(self):
    ...
    def test_dict_nocopy(
        ...
        # IMPLEMENTED: most of the rest of this test belongs in indexing tests
        ...
        if copy:
            ...
            # IMPLEMENTED: we can call check_views if we stop consolidating
            #  in setitem_with_indexer
            assert c[0] == 45  # i.e. df.iloc[0, 2]=45 *did* update c
            # IMPLEMENTED: we can check b[0] == 0 if we stop consolidating in
...
class TestDataFrameConstructorIndexInference:
    ...
    def test_frame_from_dict_with_mixed_tzaware_indexes(self):
        ...
        # IMPLEMENTED: not clear if these raising is desired (no extant tests),
...
class TestDataFrameConstructorWithDatetimeTZ:
    ...
    # IMPLEMENTED: make this not cast to object in pandas 3.0
    @pytest.mark.skipif(
...
def get1(obj):  # IMPLEMENTED: make a helper in tm?
...
# IMPLEMENTED: better location for this test?
