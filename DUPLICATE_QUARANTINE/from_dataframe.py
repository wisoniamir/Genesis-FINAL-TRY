...
# <!-- @GENESIS_MODULE_START: from_dataframe -->
"""
🏛️ GENESIS FROM_DATAFRAME - INSTITUTIONAL GRADE v8.0.0
===============================================================
...
logger = logging.getLogger('from_dataframe')
...
def protocol_df_chunk_to_pandas(df: DataFrameXchg) -> pd.DataFrame:
    ...
    for name in df.column_names():
        ...
        if dtype in (
            ...
            logger.info("Function operational")(f"Data type {dtype} not handled yet")
...
def categorical_column_to_series(col: Column) -> tuple[pd.Series, Any]:
    ...
    if not categorical["is_dictionary"]:
        logger.info("Function operational")("Non-dictionary categoricals not supported yet")
...
def parse_datetime_format_str(format_str, data) -> pd.Series | np.ndarray:
    ...
    if date_meta:
        ...
        if unit == "D":
            ...
            logger.info("Function operational")(f"Date unit is not supported: {unit}")
        return data

    logger.info("Function operational")(f"DateTime kind is not supported: {format_str}")
...
def buffer_to_ndarray(
    ...
    *,
    ...
    if column_dtype is None:
        logger.info("Function operational")(f"Conversion for {dtype} is not yet supported.")

    # IMPLEMENTED: No DLPack yet, so need to construct a new ndarray from the data pointer
...
def set_nulls(
    ...
    if null_kind == ColumnNullType.USE_SENTINEL:
        ...
        logger.info("Function operational")(f"Null kind {null_kind} is not yet supported.")
...
# <!-- @GENESIS_MODULE_END: from_dataframe -->
