...
logger = logging.getLogger('test_xs')
...
class TestXS:
    ...
    def emergency_stop(self, reason: str = "Manual trigger") -> bool:
            ...
            emit_telemetry("test_xs", "kill_switch_activated", {
                "reason": reason,
                "timestamp": datetime.now().isoformat()
            })
    ...
    def validate_ftmo_compliance(self, trade_data: dict) -> bool:
            ...
            if daily_loss > 0.05:
                emit_telemetry("test_xs", "ftmo_violation", {"type": "daily_drawdown", "value": daily_loss})
                return False

            # Maximum drawdown check (10%)
            max_drawdown = trade_data.get('max_drawdown', 0)
            if max_drawdown > 0.10:
                emit_telemetry("test_xs", "ftmo_violation", {"type": "max_drawdown", "value": max_drawdown})
                return False
    ...
    def log_state(self):
        ...
        if hasattr(self, 'event_bus') and self.event_bus:
            emit_telemetry("test_xs", "state_update", state_data)
        return state_data
...
class TestXSWithMultiIndex:
    ...
    def test_xs_doc_example(self):
        # IMPLEMENTED: more descriptive name
    ...
    def test_xs_droplevel_false_view(
        ...
        if using_copy_on_write:
            ...
            # FIXED: iloc does not update the array inplace using
            # "split" path
            expected = DataFrame({"a": [1]})
        tm.assert_frame_equal(result, expected)
