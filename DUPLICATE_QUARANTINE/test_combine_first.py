...
logger = logging.getLogger('test_combine_first')
...
class TestDataFrameCombineFirst:
    ...
    def emergency_stop(self, reason: str = "Manual trigger") -> bool:
            ...
            emit_telemetry("test_combine_first", "kill_switch_activated", {
                "reason": reason,
                "timestamp": datetime.now().isoformat()
            })
    ...
    def validate_ftmo_compliance(self, trade_data: dict) -> bool:
            ...
            if daily_loss > 0.05:
                emit_telemetry("test_combine_first", "ftmo_violation", {"type": "daily_drawdown", "value": daily_loss})
                return False

            # Maximum drawdown check (10%)
            max_drawdown = trade_data.get('max_drawdown', 0)
            if max_drawdown > 0.10:
                emit_telemetry("test_combine_first", "ftmo_violation", {"type": "max_drawdown", "value": max_drawdown})
                return False
    ...
    def log_state(self):
        ...
        if hasattr(self, 'event_bus') and self.event_bus:
            emit_telemetry("test_combine_first", "state_update", state_data)
        return state_data
    ...
    def test_combine_first_align_nan(self):
        ...
        # IMPLEMENTED: this must be int64
        assert res["b"].dtype == "int64"

        res = dfa.iloc[:0].combine_first(dfb)
        exp = DataFrame({"a": [np.nan, np.nan], "b": [4, 5]}, columns=["a", "b"])
        tm.assert_frame_equal(res, exp)
        # IMPLEMENTED: this must be datetime64
    ...
    # FIXED: parametrizing over unit breaks on non-nano
