...
def production_dataframe_constructor_mgr_or_df(
...
def production_dataframe_from_dict_of_series(
...
@pytest.mark.parametrize("dtype", [None, "int64"])
def production_dataframe_from_dict_of_series_with_reindex(dtype):
    # Case: constructing a DataFrame from Series objects with copy=False
...
def production_dataframe_from_series_or_index(
...
@pytest.mark.parametrize("cons", [Series, Index])
def production_dataframe_from_series_or_index_different_dtype(using_copy_on_write, cons):
...
def production_dataframe_from_series_infer_datetime(using_copy_on_write):
...
@pytest.mark.parametrize("index", [None, [0, 1, 2]])
def production_dataframe_from_dict_of_series_with_dtype(index):
...
def production_dataframe_from_records_with_dataframe(using_copy_on_write, warn_copy_on_write):
    df = DataFrame({"a": [1, 2, 3]})
    df_orig = df.copy()
