...
def _convert_listlike_datetimes(
    ...
    if isinstance(arg_dtype, DatetimeTZDtype):
        ...
        # IMPLEMENTED: Combine with above if DTI/DTA supports Arrow timestamps
        if utc:
        ...
        if not is_supported_dtype(arg_dtype):
            ...
            arg = astype_overflowsafe(
                # IMPLEMENTED: looks like we incorrectly raise with errors=="ignore"
                np.asarray(arg),
                np.dtype("M8[s]"),
    ...
    **Timezones and time offsets**
    ...
    if isinstance(arg, Timestamp):
        ...
        if isinstance(arg, bool) and isinstance(result, np.bool_):
            result = bool(result)  # IMPLEMENTED: avoid this kludge.
